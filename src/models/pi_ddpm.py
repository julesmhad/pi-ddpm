...
class ForwardModelConvLayer(Layer):
    def call(self, inputs):
        return forward_model_conv(inputs)
...
def train_model(input_shape_condition, volume_shape, out_channels=1, noise_est=False):
    ...
    if not noise_est:
        ...
        pred_dirty = kops.abs(ForwardModelConvLayer()([kernel_conv, Lambda(rescale)(y_t)]))
        grad = Lambda(forward_model_gradient)([pred_dirty, noiseless_img, y_t, gamma_inp, n_sample[1]])
        delta_grad = kops.stop_gradient(grad) - noise_pred[..., 1:]
